...
export function OrientacoesSection({ data, onAdd, onEdit, onDelete }: any) {
  ...
  const opcoesTipo = [
    ...
    { label: "Trabalho de Conclusão de Curso", value: "TCC" },
    { label: "Mestrado", value: "MESTRADO" },
    { label: "Doutorado", value: "DOUTORADO" },
    { label: "Pós-Doutorado", value: "POS_DOUTORADO" },
    { label: "Supervisão de Estágio", value: "SUPERVISAO_ESTAGIO" },
  ];

  return (
    <PerfilAcademicoSectionLayout title="Orientações">
      ...
      {/* Empty State - Quando não há orientações */}
      {data.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 px-4 mt-4">
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-8 max-w-md w-full text-center">
            ...
            <p className="text-gray-600 text-sm">
              Adicione suas orientações para enriquecer seu perfil e destacar sua atuação como orientador.
            </p>
          </div>
        </div>
      )}
