...
export function AtuacoesSection({ data, onAdd, onEdit, onDelete }: any) {
  ...
  return (
    <PerfilAcademicoSectionLayout title="Atuações Profissionais">
      ...
      <div className="flex justify-center pt-2">
        ...
        >
          + Adicionar Atuação Profissional
        </button>
      </div>

      {/* Empty State - Quando não há atuações */}
      {data.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 px-4 mt-4">
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-8 max-w-md w-full text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">
              Nenhuma atuação profissional cadastrada
            </h3>
            <p className="text-gray-600 text-sm">
              Adicione suas atuações profissionais para enriquecer seu perfil e destacar sua trajetória profissional.
            </p>
          </div>
        </div>
      )}
